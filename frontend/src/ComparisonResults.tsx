...
import type { AnalysisResult, Comparison, Evidence, Finding, Fragment, FunctionChange, FunctionRecord, ReviewRecord, ReviewStatus, UnitChange, WorkspaceDocument } from './lib/types';
import { documentCount, filteredFindings, filterChanges, findingLabels } from './lib/presentation';
import type { FindingSort } from './lib/presentation';
import { useStoredChoice } from './lib/preferences';
...
function FindingCard({ finding, review, comparison, relatedFunctions, onOpen, onRefresh }: { finding: Finding; review?: ReviewRecord; comparison: Comparison; relatedFunctions: FunctionRecord[]; onOpen: (evidence: Evidence) => void; onRefresh: () => Promise<void> }) {
  ...
  return <article className={`panel analysis-finding finding-${finding.kind}`} aria-labelledby={`finding-${finding.id}`}>
    ...
    {relatedFunctions.length > 0 && <div className="finding-functions" aria-label="Функции, связанные с выводом">{relatedFunctions.map((item) => <div key={item.id}><span className="eyebrow">{sideLabels[item.side]}</span><FunctionDetails item={item} /></div>)}</div>}
...
export default function ComparisonResults({ comparison, result, documents = [], documentsLoading = false, onRefresh }: Props) {
  const [tab, setTab] = useStoredChoice(`larpsec.resultTab.${comparison.id}`, ['findings', 'functions', 'units', 'repetitions'] as const, 'findings');
  const [kindFilter, setKindFilter] = useState<Finding['kind'] | 'all'>('all');
  const [reviewFilter, setReviewFilter] = useState<ReviewStatus | 'all'>('all');
  const [sort, setSort] = useState<FindingSort>('original');
  const [functionFilter, setFunctionFilter] = useState<FunctionChange['status'] | 'all'>('all');
  const [unitFilter, setUnitFilter] = useState<UnitChange['status'] | 'all'>('all');
  ...
  const visibleFindings = filteredFindings(result, kindFilter, reviewFilter, sort);
  const visibleFunctions = filterChanges(result.function_changes, functionFilter);
  const visibleUnits = filterChanges(result.unit_changes, unitFilter);
  ...
  return <section className="analysis-results" aria-label="Результат сравнения">
    ...
    <div className="analysis-stats" aria-label="Сводка замечаний">{(Object.keys(findingLabels) as (keyof typeof findingLabels)[]).map((kind) => <button type="button" className={`panel analysis-stat stat-${kind}`} key={kind} aria-pressed={tab === 'findings' && kindFilter === kind} onClick={() => { setTab('findings'); setKindFilter(kindFilter === kind ? 'all' : kind); setReviewFilter('all'); }}><span>{findingLabels[kind]}</span><strong>{result.findings.filter((finding) => finding.kind === kind).length}</strong></button>)}</div>
    <div className="analysis-text-tools" aria-label="Дополнительная проверка текста"><p>Проверка текста · отдельно от замечаний о функциях</p><button type="button" className="panel analysis-stat stat-repetitions" aria-pressed={tab === 'repetitions'} onClick={showRepetitions}><span>Повторы текста «после»</span><strong>{sourcesPending ? '…' : unavailableSides.includes('after') ? '—' : afterRepetitions.length}</strong></button></div>
    <div className="panel analysis-summary"><p>{result.summary}</p><div className="analysis-summary-meta"><span>{documentCount(comparison.documents.length)}</span><span>{result.functions.filter((item) => item.side === 'before').length} функций до → {result.functions.filter((item) => item.side === 'after').length} после</span><span>{new Date(result.generated_at).toLocaleString('ru-RU')}</span></div><div className="analysis-coverage">{(['before', 'after'] as const).map((side) => <span key={side} className={result.coverage[side] ? 'coverage-complete' : 'coverage-incomplete'}>{result.coverage[side] ? <Check size={14} /> : <AlertCircle size={14} />}{sideLabels[side]}: {result.coverage[side] ? 'полное покрытие' : 'неполное покрытие'}</span>)}</div>{(!result.coverage.before || !result.coverage.after) && <p className="analysis-coverage-note">При неполном наборе документов или неполном извлечении текста нельзя уверенно судить о потере функций.</p>}</div>
    ...
    {tab === 'findings' && <div className="finding-filters">
      <label>Тип замечания<select aria-label="Тип замечания" value={kindFilter} onChange={(event) => setKindFilter(event.target.value as Finding['kind'] | 'all')}><option value="all">Все типы</option>{(Object.keys(findingLabels) as Finding['kind'][]).map((kind) => <option key={kind} value={kind}>{findingLabels[kind]}</option>)}</select></label>
      <label>Проверка экспертом<select aria-label="Проверка экспертом" value={reviewFilter} onChange={(event) => setReviewFilter(event.target.value as ReviewStatus | 'all')}><option value="all">Все решения</option>{(Object.keys(reviewLabels) as ReviewStatus[]).map((status) => <option key={status} value={status}>{reviewLabels[status]}</option>)}</select></label>
      <label>Порядок выводов<select aria-label="Порядок выводов" value={sort} onChange={(event) => setSort(event.target.value as FindingSort)}><option value="original">Исходный порядок</option><option value="kind">По типу замечания</option><option value="review">Сначала непроверенные</option></select></label>
      <span role="status">Показано {visibleFindings.length} из {result.findings.length}</span>
      {(kindFilter !== 'all' || reviewFilter !== 'all') && <button className="text-button" onClick={() => { setKindFilter('all'); setReviewFilter('all'); }}>Сбросить фильтры</button>}
    </div>}
    {tab === 'functions' && <div className="finding-filters"><label>Статус функции<select aria-label="Статус функции" value={functionFilter} onChange={(event) => setFunctionFilter(event.target.value as FunctionChange['status'] | 'all')}><option value="all">Все функции</option>{(Object.keys(functionLabels) as FunctionChange['status'][]).map((status) => <option key={status} value={status}>{functionLabels[status]}</option>)}</select></label><span role="status">Показано {visibleFunctions.length} из {result.function_changes.length}</span></div>}
    {tab === 'units' && <div className="finding-filters"><label>Статус подразделения<select aria-label="Статус подразделения" value={unitFilter} onChange={(event) => setUnitFilter(event.target.value as UnitChange['status'] | 'all')}><option value="all">Все подразделения</option>{(Object.keys(unitLabels) as UnitChange['status'][]).map((status) => <option key={status} value={status}>{unitLabels[status]}</option>)}</select></label><span role="status">Показано {visibleUnits.length} из {result.unit_changes.length}</span></div>}
    <div id="analysis-content" ref={content} tabIndex={-1} className="analysis-content" role="region" aria-label={tab === 'findings' ? 'Выводы сравнения' : tab === 'functions' ? 'Изменения функций' : tab === 'units' ? 'Изменения подразделений' : 'Повторы текста'}>
      {tab === 'findings' && (visibleFindings.length ? visibleFindings.map((finding) => <FindingCard key={finding.id} finding={finding} review={reviews.get(finding.id)} comparison={comparison} relatedFunctions={finding.function_ids.flatMap((id) => { const item = functions.get(id); return item ? [item] : []; })} onOpen={setSource} onRefresh={onRefresh} />) : <div className="panel analysis-empty"><CheckCheck size={28} /><h3>{result.findings.length ? 'Нет замечаний с выбранными фильтрами' : 'Замечаний не найдено'}</h3><p>{result.findings.length ? 'Выберите другой тип или статус проверки, чтобы увидеть остальные выводы.' : 'Посмотрите сопоставление функций и подразделений. Отсутствие замечаний относится к загруженным документам и возможностям выбранного режима анализа.'}</p>{kindFilter === 'duplication' && <><p>Дублирование функций относится к полномочиям разных владельцев. Дословные повторы пунктов показаны отдельно.</p><button className="button secondary" onClick={showRepetitions}>Показать повторы текста</button></>}</div>)}
      ...
      {tab === 'functions' && (visibleFunctions.length ? visibleFunctions.map((change, index) => {
