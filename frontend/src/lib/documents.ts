...
export const DOCUMENT_ACCEPT = '.docx,.pdf,.xlsx,.txt';
export const DOCUMENT_FORMATS = 'DOCX, PDF, XLSX и TXT';

export function validateFile(file: Pick<File, 'name' | 'size'>): string | undefined {
  if (/\.xls$/i.test(file.name)) return 'Для Excel поддерживается XLSX. Откройте файл XLS в Excel и сохраните как «Книга Excel (.xlsx)».';
  if (!/\.(docx|pdf|xlsx|txt)$/i.test(file.name)) return `Поддерживаются ${DOCUMENT_FORMATS}. Сохраните документ в одном из этих форматов.`;
...
export async function validateDocumentSignature(file: File) {
  // Plain text has no magic signature. The server validates its encoding and contents.
...
export function downloadText(name: string, text: string) {
  ...
  link.href = url;
  link.download = name.replace(/\.(docx|pdf|xlsx|txt)$/i, '') + '.txt';
  link.click();
