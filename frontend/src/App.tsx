...
export default function App() {
  ...
  const analysisFailed = comparison?.status === 'failed' && comparison.documents.length > 0;
  const analysisPending = comparison?.status === 'queued' || comparison?.status === 'running';
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-bottom"><button className="help-button" onClick={() => setGuide(true)}><CircleHelp size={18} />Как это работает<ArrowUpRight size={16} /></button><div className="sidebar-user"><span className="avatar">L</span><div><strong>Команда Larpsec</strong><span>HackAlem AI</span></div><ShieldCheck size={17} /></div></div>
    </aside>
    <div className="main-shell">
      <header className="topbar"><div className="breadcrumb"><PanelLeftClose size={18} /><span>Рабочее пространство</span><ChevronRight size={14} /><strong>{view === 'history' ? 'История' : view === 'comparison' ? 'Сравнение' : 'Документы'}</strong></div><div className="topbar-right"><ThemeToggle /><span className="event-tag">HACKALEM <span>AI</span></span><span className="header-divider" /><span className="avatar small-avatar">L</span></div></header>
      <main id="main">
        ...
        {view !== 'history' && <ComparisonControls showAnalysisError={view === 'comparison' && comparisonView === 'analysis'} comparison={comparison} documents={documents} health={health} result={result} disabled={unavailable} loading={loading || working} beforeComplete={beforeComplete} afterComplete={afterComplete} setBeforeComplete={setBeforeComplete} setAfterComplete={setAfterComplete} onAnalyze={() => void startAnalysis()} onNew={() => void startNew()} onShow={showAnalysis} onTextDiff={showTextComparison} onSample={() => void samplePair()} />}
        {view === 'history' ? <ComparisonHistory activeId={comparison?.id} disabled={busy || working || choosing || loading} onOpen={async (item) => { await openComparison(item.id); setSelectedId(null); setView('comparison'); setComparisonView(item.status === 'completed' ? 'analysis' : 'text'); }} onDeleted={async (id) => { if (comparison?.id === id) { await newComparison(); setSelectedId(null); } }} /> : view === 'comparison' ? <div className="comparison-workspace">
          ...
          {comparisonView === 'text' ? <DocumentDiff key={comparison?.id || 'empty'} documents={documents} onDocuments={newDocument} /> : result && comparison ? <ComparisonResults key={comparison.id} comparison={comparison} result={result} documents={documents} documentsLoading={loading} onRefresh={refresh} /> : <section className="comparison-empty panel"><GitCompareArrows size={30} /><h2>{analysisPending ? 'Сервер обрабатывает документы' : analysisFailed ? 'Анализ функций не завершён' : 'Анализ функций ещё не запущен'}</h2><p>{analysisPending ? 'Статус обновляется автоматически. Можно обновить страницу — обработка продолжится на сервере.' : analysisFailed ? 'Проверьте документы и повторите анализ кнопкой выше. Сравнение текста доступно в соседней вкладке.' : 'Загрузите документы обеих версий и запустите сравнение кнопкой выше. Подсветка изменений текста уже доступна в соседней вкладке.'}</p><button className="button secondary" onClick={newDocument}>К документам <ArrowRight size={16} /></button></section>}
