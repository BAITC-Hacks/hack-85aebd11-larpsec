...
export function ComparisonControls(props: Props) {
  ...
  const failed = comparison?.status === 'failed' && comparison.documents.length > 0;
  const displayFailure = failed && props.showAnalysisError;
  const stage = comparison?.status === 'failed' && !displayFailure ? 'draft' : comparison?.stage || 'draft';
  return <section className="comparison-controls panel" aria-label="Подготовка сравнения">
    <div className="comparison-control-heading"><div><span className="eyebrow">ДОКУМЕНТЫ → АНАЛИЗ → ИСТОЧНИКИ</span><h2><GitCompareArrows size={22} />{stages[stage] || 'Анализ документов'}</h2></div><button className="text-button" disabled={newDisabled} onClick={props.onNew}><Plus size={16} />Новое сравнение</button></div>
    ...
    {displayFailure && <div className="inline-error" role="alert"><p>{comparison.error?.message || 'Анализ завершился с ошибкой. Повторите запуск.'}</p></div>}
    <div className="comparison-actions">
      ...
      {result ? <button className="button primary" onClick={props.onShow}>Открыть результаты <ArrowRight size={17} /></button> : !active && <button className="button primary" disabled={disabled || !canAnalyze} onClick={props.onAnalyze}>{loading ? <LoaderCircle className="spin" size={17} /> : <GitCompareArrows size={17} />}{failed ? 'Повторить сравнение' : 'Запустить сравнение'}</button>}
