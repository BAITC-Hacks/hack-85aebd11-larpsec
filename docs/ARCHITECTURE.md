...
```mermaid
flowchart LR
    Browser[Браузер] --> Static[FastAPI: frontend/dist]
    Static --> UI[React + TypeScript]
    UI --> API[FastAPI /api/v1]
    API --> Files[Локальные оригиналы]
    API --> Parse[DOCX / PDF / XLSX / TXT]
...
- `frontend/src`: интерфейс документов, состояния анализа, сопоставлений и источников.
- `run.sh`: воспроизводимая установка зависимостей, сборка интерфейса и запуск сервера.
...
## Подключение интерфейса и запуск

`./run.sh` использует зафиксированную в `frontend/package.json` версию pnpm через npx,
собирает `frontend/dist` и запускает `backend/run.sh`. Статические файлы монтируются
в FastAPI последними: `/api/v1`, `/health`, `/docs` и `/openapi.json` сохраняют свои
обработчики. Сайт и API доступны по одному origin на `http://127.0.0.1:8000`.
Неизвестные URL API и отсутствующие assets возвращают HTTP 404, без подмены на index.html.
Если `frontend/dist/index.html` отсутствует, бэкенд запускается самостоятельно без сайта.
Сборка должна существовать до запуска процесса; `./run.sh` обеспечивает этот порядок.

В разработке Vite на 5173 проксирует `/api` и `/health` на `API_PROXY_TARGET` (по умолчанию
порт 8000). Клиент использует `/api/v1`; `VITE_API_BASE_URL` меняется во время сборки.
Прокси Vite не участвует в production. При отдельном домене интерфейса требуется
настройка CORS на сервере. Модельный ключ хранится исключительно на бэкенде.

Docker собирает интерфейс в Node-stage, копирует `dist` в `/app/frontend/dist`,
а сервер запускает из `/app/backend`. Build context — корень репозитория;
Dockerfile расположен в `backend/Dockerfile`. `.dockerignore` исключает локальные
ключи, БД, виртуальные окружения, node_modules и результаты предыдущих сборок.
Оригиналы и SQLite сохраняются в томе `analysis_data`; необязательный `backend/.env`
передаёт параметры сервера. Без настроенного режима llm используется demo без ИИ.
...
История читается с сервера. Удаление сравнения очищает документы, файлы, результаты и оценки;
для queued/running оно запрещено. Проверка статуса и удаление сериализованы с запуском
анализа; исчезновение записи во время чтения истории не прерывает выдачу остальных записей.
...
В режиме `demo` правила распознают явные обозначения владельцев, заголовки положений
и функций, нумерованные и маркированные пункты. Основные окончания первого глагола
нормализуются; вид нормы, область и отрицание сохраняются. Точное текущее имя владельца
имеет приоритет перед чужим прежним названием. Явные указания прежнего имени позволяют
отделить переименование владельца от передачи функции. Это не семантический ИИ:
свободные перефразирования, неявные слияния, разделения и сложные конфликты требуют
режима llm и проверки качества. Конфликты между должностью и подразделением через
выведенную связь полномочий в демонстрационных правилах не реализованы.

Подчинение сравнивается отдельно по субъекту и виду, с учётом связей переименования.
Изменения возвращаются как `reporting_change` с цитатами; двойное функциональное и
административное подчинение не считается противоречием само по себе. Если функции
не распознаны, возвращается завершённый результат с `coverage_gap` и предупреждениями;
доступные сведения о структуре и подчинении продолжают сопоставляться.

Markdown-отчёт содержит русские подписи, даты и источники. Самостоятельный JSON-экспорт
дополняет результат метаданными сравнения и документов, полными исходными фрагментами
и координатами каждой цитаты. Компактный API `/result` при этом сохраняется.
Общий `API_TOKEN`, если задан, защищает API и маршруты документации `/docs`, `/redoc`
и `/openapi.json`; схема не открывается анонимно.

## Документы и ограничения

- Word: `.docx`, основной текст, таблицы, колонтитулы; восстанавливаются номера списков
  и наследование стилей. Адрес — раздел/абзац/пункт, без выдуманных страниц.
...
- Excel: `.xlsx`, текст ячеек, листы и диапазоны; таблицы с известными заголовками
  преобразуются в функции с владельцами. Неоднозначные таблицы сохраняют исходные
  значения с предупреждением о полноте; формулы не вычисляются.
- TXT: UTF-8 (с BOM или без), UTF-16/UTF-32 с BOM, Windows-1251 с предупреждением.
  Источники адресуются номерами строк; бинарное содержимое отклоняется.
...
- Комплект ограничен числом файлов/символов/функций. По умолчанию: 20 файлов,
  20 МБ на файл, 200000 символов на документ, 400000 на сравнение, 1000 функций на каждую сторону сравнения.
